...
\minisec{The |PUBSTATE| field}
The |PUBSTATE| field is only used for the citation and references
year if there is no |labelyear| field. This makes sense as the |PUBSTATE|
field is for things not officially yet published and thus without a
publication year. However, you may wish to have a date field in such an
entry too (perhaps you know when it will be published, for example). You
can easily deal with this in |biber| by using a |biber.conf| file to remove
date fields from entries with a |PUBSTATE| field dynamically as the data
source stream is being processed. For example, to ignore some common date
fields in such entries, your |biber.conf| might contain:
...
<config>
  <sourcemap>
    <maps datatype="bibtex" map_overwrite="0">
      <map>
        <map_step map_field_source="PUBSTATE"/>
