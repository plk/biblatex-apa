...
(\path{apa.bbx}) and a string localisation file (\path{american-apa.lbx})
for «american» English. The styles are loaded just like any other
\sty{biblatex} styles but I wouldn't try to use the citation and references
styles separately as they rely on each other, macro-wise, in places.
...
\path{<texmf>/tex/latex/biblatex/lbx/american-apa.lbx}\\
...
document preamble, after loading \sty{biblatex}. Here is an example to load
the German localisation strings. This assumes that you are using the
\sty{babel} package with the appropriate language option:
...
file and augments it with APA specific strings.
...
\item Fixed bug were multiple year ranges were not displayed properly.
\item Updated to remove pre-\sty{biblatex} 0.8e macros error.
\end{release}

\begin{release}{0.5}{2009-09-12}
\item Replaced literal string with localised form in url macro
\item Moved localisation strings into \sty{lbx} file using |\DeclareLanguageMapping|
\end{release}
